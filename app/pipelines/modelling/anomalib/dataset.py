"""Dataset partitioning and seeding utilities for Anomalib pipelines."""
...
def seed_anomalib_run(seed: int) -> None:
    """Seed every random source used by Anomalib and its data loaders.
...
def dataset_with_ordered_paths(dataset: Any, ordered_paths: list[str]) -> Any:
    ...
    if samples is None or "image_path" not in samples.columns:
        raise TypeError("Anomalib dataset must expose a samples frame with image_path")
    indexed = {
...
def configure_anomalib_partitions(
    ...
    train_data = dataset_with_ordered_paths(source_train, fair_split.fitting_paths)
    validation_data = dataset_with_ordered_paths(source_train, fair_split.validation_paths)
    test_data = dataset_with_ordered_paths(source_test, fair_split.test_paths)
