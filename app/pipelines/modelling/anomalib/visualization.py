"""Shared Anomalib visualization and artifact serialization utilities."""
...
class RawScoreImageVisualizer(ImageVisualizer):
    ...
    def on_test_batch_end(
        ...
        for item in visualization_batch:
            ...
            if image is None:
                continue
            image = add_panel_headers(image, labels, panel_width=self.field_size[0])
...
def print_anomalib_results_table(results: Mapping[str, float]) -> None:
    """Print corrected evaluation metrics in Anomalib's table layout."""
...
def load_heatmap_overlays(input_path: Path) -> dict[int, dict[str, list[Any]]]:
...
def normalize_overlay_image(img: np.ndarray[Any, Any]) -> np.ndarray[Any, Any]:
...
def normalize_percentile_amap(amap: np.ndarray[Any, Any]) -> np.ndarray[Any, Any]:
...
def create_sample_heatmap_overlay(
    ...
    orig_img = normalize_overlay_image(img)
    amap_norm = normalize_percentile_amap(amap)
...
def process_batch_heatmaps(
    ...
    for i, label in enumerate(gt_labels_np):
        if int(label) == 1:
            ...
            hm_small, gt_hm_small = create_sample_heatmap_overlay(images_np[i], anomaly_maps_np[i], mask_i)
...
def generate_test_heatmaps(
    ...
    try:
        ...
        for batch in predictions:
            global_idx = process_batch_heatmaps(batch, global_idx, heatmap_overlays, anomalous_indices)
