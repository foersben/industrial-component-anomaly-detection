...
* **The Concept:** A boxplot comparing baseline model scores (such as AUPIMO or AU-PRO) across the 15 distinct categories of the MVTec AD dataset.
...
* **Statistical Validation:** Performs a **One-Way ANOVA** (or Kruskal-Wallis if variance normality assumptions fail) on the performance scores across the 15 categories to yield a p-value, proving that the performance differences between texture and object categories are statistically significant.
