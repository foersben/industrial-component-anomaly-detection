...
where $x_i$ represents an individual image within the held-out normal validation set $\mathcal{D}_{\mathrm{val, normal}}$, $s_i$ denotes the corresponding continuous anomaly score computed by the model for that image, and $\alpha$ denotes the plant's tolerable false-alarm budget (the Type I error ceiling). Specifying $\alpha = 0.005$ establishes an empirical threshold $\tau_{\alpha}$ ensuring that exactly $99.5\%$ of conforming components within the calibration population pass the gate without intervention. 

Crucially, $\alpha$ is not an arbitrary hyperparameter; it is inextricably linked to the economic optimal threshold $t^*$ defined in Equation~\ref{eq:optimal_threshold}. Under the assumption that the financial loss of false alarms and defect escapes must be minimised, we directly set the false-alarm budget $\alpha = t^*$. This precisely maps the theoretical cost-derived ratio $t^*$ (a probability) directly into the empirical score boundary $\tau_\alpha$ (a physical threshold quantile) on the factory floor, ensuring the operational defect divert rate strictly obeys the plant's cost-of-quality model.
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item \textbf{Tier 2: Precision Industrial (e.g., Automotive Powertrain):} Governed by a balanced $F_1$-optimised objective ($\alpha \approx 0.05$). We establish a cost-neutral equilibrium between Type I and Type II errors, appropriate for mid-tier risk environments.
    \item \textbf{Tier 3: High-Throughput Commodity (e.g., Threaded Fasteners):} Governed by an $F_{0.5}$-optimised objective ($\alpha \approx 0.01$). We weight precision four times more heavily than defect recall, preventing pseudo-scrap from choking downstream automated packaging equipment while catching severe structural anomalies.
\end{itemize}
