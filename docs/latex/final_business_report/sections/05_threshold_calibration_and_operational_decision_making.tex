...
Figure~\ref{fig:threshold_calibration} illustrates how the three profiles select different quantiles of the same normal-validation distribution. The curves are schematic because the final report retains the frozen thresholds and metrics, but not the underlying per-image validation-score arrays.

\begin{figure}[tp]
    \centering
    \includegraphics[width=\linewidth]{figures/business_threshold_tiers.pdf}
    \caption{Schematic relationship between the normal-validation score distribution and the three predeclared operating profiles. The reported benchmark uses the Tier 2 setting, $\alpha=0.05$; no empirical result is inferred from the illustrative curve shapes.}
