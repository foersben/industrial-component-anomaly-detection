...
Industrial IPC (Core i7, AVX2 + oneDNN) & 1 & 65 -- 95\,ms & 10 -- 15 \\
Standard CPU Fallback (scalar) & 1 & 320 -- 450\,ms & 2 -- 3 \\
\bottomrule
\end{tabular}
\end{table}

The CPU execution profile constitutes the most commercially significant result in this benchmarking suite. By leveraging Intel oneDNN operator fusion and eight-wide AVX2 SIMD vectorisation, the ResNet-18 feature extraction and nearest-neighbour distance query execute in 65--95\,ms on a standard, fanless industrial IPC equipped with an Intel Core i7 processor. This achieves deterministic throughput of 10--15 parts per second on commodity edge hardware already deployed across existing plant electrical cabinets. For brownfield retrofit installations on established manufacturing lines, this completely eliminates the substantial capital expenditure, enclosure redesign, and maintenance overhead associated with dedicated industrial GPU accelerators.
