...
\end{columns}
\note{\scriptsize
  \textbf{Notation on PatchCore diagram ($\phi(x)$ and $m \in \mathcal{M}$):}\\[1pt]
  ...
  $\min_{m \in \mathcal{M}} \|\phi(x) - m\|_2$ is the nearest-neighbour $L_2$ distance: an anomaly is detected when a query patch vector has no close normal counterpart in memory.\\[3pt]
...
\end{columns}
\note{\scriptsize
  \textbf{Inference formula ($s(p^*) = \min_{m \in \mathcal{M}} \|p^* - m\|_2$):}\\[1pt]
  ...
  Scoring is non-parametric nearest-neighbour distance: if a query patch has no close match in $\mathcal{M}$, its distance is large $\Rightarrow$ anomalous.\\[3pt]
...
\begin{columns}[T]
  ...
  \begin{column}{0.38\textwidth}
    ...
    \end{center}
    \vspace{-4pt}
    {\tiny\color{muted} Explainability: anomaly heatmaps provide pixel-level attribution\\[-0.2cm]$\Rightarrow$ quality engineers review in seconds, replacing manual inspection.}\\[6pt]
