...
\begin{columns}[T]
  \begin{column}{0.55\textwidth}
    ...
    \begin{itemize}\small
      ...
      \item Normal-only training; test set has pixel-level anomaly gt
    \end{itemize}
  ...
  \begin{column}{0.48\textwidth}
    ...
    \begin{itemize}\setlength{\itemsep}{6pt}\setlength{\leftskip}{-10pt}
      ...
      \item \textbf{Architectural Choices:}\\
            \textbf{ELU} activations avoid dying neurons.\\
...
\begin{tikzpicture}[
  ...
  % ROW 1: Data Prep (L -> R)
  \node[stepbox, text width=2.5cm] (data) at (0, 0) {\textbf{1. MVTec Dataset}};
  \node[stepbox, text width=1.8cm, fill=accent!5, draw=accent] (cat) at (3.1, 0) {\textbf{2. Category?}};
  \node[stepbox, text width=2.0cm, fill=teal!15] (tex) at (5.7, 0.6) {\textbf{Texture}\\Rotate+Flip};
  \node[stepbox, text width=2.0cm, fill=accent!15] (obj) at (5.7, -0.6) {\textbf{Object}\\Colour+Noise};
  \node[stepbox, text width=2.5cm] (seg) at (8.7, 0) {\textbf{3. Fg. Mask}\\Otsu+Canny};

  % Data to Cat
  \draw[arr] (data) -- (cat);

  % 1. Branch: Cat -> Tex & Obj (- | ->)
  \draw[thick, color=teal!80] (cat.east) -- ++(0.1, 0) coordinate (fork);
  \draw[arr] (fork) |- (tex.west);
  \draw[arr] (fork) |- (obj.west);

  % 2. Merge: Tex & Obj -> Seg (- | ->)
  \path (seg.west) ++(-0.35, 0) coordinate (join);
  \draw[thick, color=teal!80] (tex.east) -| (join);
  \draw[thick, color=teal!80] (obj.east) -| (join);
  \draw[arr] (join) -- (seg.west);

  % ROW 2: Model (R -> L)
  \node[stepbox, text width=3.0cm, fill=teal!10] (build) at (8.7, -2.0) {\textbf{4. Build Keras CAE}\\ELU + BN + AdamW};
  \node[stepbox, text width=3.0cm, fill=teal!20] (train) at (4.3, -2.0) {\textbf{5. Train Model}\\SSIM+MSE + MIM};
  \node[stepbox, text width=3.0cm] (score) at (0, -2.0) {\textbf{6. Score Images}\\Top-K Pooling};
  ...
  % ROW 3: Inference (L -> R)
  \node[stepbox, text width=3.0cm] (thresh) at (0, -3.6) {\textbf{7. Adapt. Threshold}\\Quantile/Mahalanobis};
  \node[stepbox, text width=3.0cm, fill=accent!10, draw=accent] (eval) at (4.3, -3.6) {\textbf{8. Evaluation}\\AUROC + AUPIMO};
