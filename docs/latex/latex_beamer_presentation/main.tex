...
\begin{columns}[T]
  ...
  \begin{column}{0.42\textwidth}
    ...
    \begin{tabular}{ccc}
      ...
      \includegraphics[width=1.7cm,keepaspectratio]{problem_capsule_squeeze_mask.png} \\
      {\tiny\color{muted} Grid glue strip} &
      {\tiny\color{muted} Leather cut} &
      {\tiny\color{muted} GT mask example} \\
    \end{tabular}
    \end{center}
  \end{column}
\end{columns}
...
\begin{columns}[T]
  ...
  \begin{column}{0.6\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth]{fig_v2_localization_examples.png}
    \end{center}
    \vspace{-0.2cm}
    {\tiny\color{muted}\centering PatchCore anomaly localisations - defect contours outlining the identified anomaly regions.\par}
    \vspace{0.3cm}
...
\begin{tikzpicture}[
  ...
  arr/.style={-{Latex[length=2.5mm]}, thick, color=teal!80}
]
  ...
  \node[stepbox, text width=2.2cm, fill=teal!15] (tex) at (8.7, 0.7) {\textbf{Texture}\\Rotate+Flip};
  \node[stepbox, text width=2.2cm, fill=accent!15] (obj) at (8.7, -0.7) {\textbf{Object}\\Colour+Noise};
  ...
  % ROW 2: Model (R -> L)
  \node[stepbox, text width=2.4cm, fill=teal!5] (mim) at (8.7, -2.4) {\textbf{4. Masking}\\MIM Input Drop};
  \node[stepbox, text width=2.4cm, fill=teal!10] (build) at (4.35, -2.4) {\textbf{5. Build CAE}\\ELU+BN+AdamW};
  \node[stepbox, text width=2.4cm, fill=teal!20] (train) at (0, -2.4) {\textbf{6. Train Model}\\Loss: SSIM+MSE};
  ...
  % ROW 3: Inference (L -> R)
  \node[stepbox, text width=2.4cm] (score) at (0, -4.2) {\textbf{7. Score Images}\\Top-K Pooling};
  \node[stepbox, text width=2.4cm] (thresh) at (4.35, -4.2) {\textbf{8. Adapt. Thresh}\\Quantile/Mahal.};
  \node[stepbox, text width=2.4cm, fill=accent!10, draw=accent] (eval) at (8.7, -4.2) {\textbf{9. Evaluation}\\AUROC+AUPIMO};
