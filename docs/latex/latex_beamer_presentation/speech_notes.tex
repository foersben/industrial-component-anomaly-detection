...
\say{We started with ReLU. The dying neuron problem hit us fast: neurons in the bottleneck locked to zero and stopped contributing gradients entirely. We tried Leaky ReLU -- it fixes dying neurons, but it has a kink at zero, a non-smooth gradient, which introduced noise in the fine-grained reconstruction. ELU is smooth everywhere, saturates for large negative inputs, and has a zero-mean activation property that acts like internal normalisation. That is the final choice.}
...
\subsubsection{Training loss vs. inference score -- the important distinction}

\say{Here is an important point. During training the loss is SSIM plus MSE:
\[
  \mathcal{L}_\text{train} = 0.84\,(1-\mathrm{SSIM}) + 0.16\,\mathrm{MSE}
\]
MSE is differentiable and stable for gradient-based weight updates.

At inference, we switch to SSIM plus MAE for the per-pixel error map:
\[
  \text{Score} = \mathrm{TopK}_{0.2\%}\!\bigl(0.84\,(1-\mathrm{SSIM}) + 0.16\,\mathrm{MAE}\bigr)
\]
This is intentional. MAE is less sensitive to isolated large-error outlier pixels than MSE, which makes the spatial heatmap more interpretable and better calibrated for localisation. MSE at inference would produce noisier, less spatially clean heatmaps.}

\warn{If asked whether SSIM+L2 for the loss and SSIM+MAE for the score is a mistake: it is not. Training uses MSE -- differentiable, stable. Inference uses MAE -- robust, spatially interpretable. Two different contexts, two different objectives. Confirm this is correct and deliberate.}
...
\say{Raw MVTec data goes through preprocessing -- Otsu, CLAHE, blur -- then branches on category type. Textures get rotation and flip augmentations; objects get colour jitter and noise. This category-aware branching matters because texture datasets are rotationally invariant while object datasets are not.}
